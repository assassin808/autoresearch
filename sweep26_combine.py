"""Sweep 26: Combine best findings from sweep25.

New base: LR=0.03, batch=64K, WD=0.05, mom=0.97, warmdown=0.75, coupledWD.
Best single run: 3.468.

Top findings from sweep25 that may stack:
- warmdown0.8: 3.476 (on LR=0.04 base)
- WD0.03: 3.481 (on LR=0.04 base)
- LR0.03: 3.468 (on LR=0.04 base)

Now test combinations on the LR=0.03 base.
"""
...
experiments = [
    # Reference (LR=0.03, batch=64K, WD=0.05, mom=0.97, warmdown=0.75)
    ("comb: reference", [], None),

    # + warmdown0.8
    ("comb: warmdown0.8", [("WARMDOWN_RATIO = 0.75", "WARMDOWN_RATIO = 0.8")], None),

    # + WD0.03
    ("comb: WD0.03", [("WEIGHT_DECAY = 0.05", "WEIGHT_DECAY = 0.03")], None),

    # + warmdown0.8 + WD0.03
    ("comb: wd0.8+WD0.03", [
        ("WARMDOWN_RATIO = 0.75", "WARMDOWN_RATIO = 0.8"),
        ("WEIGHT_DECAY = 0.05", "WEIGHT_DECAY = 0.03"),
    ], None),

    # + warmdown0.85 (push further)
    ("comb: warmdown0.85", [("WARMDOWN_RATIO = 0.75", "WARMDOWN_RATIO = 0.85")], None),

    # + WD0.02
    ("comb: WD0.02", [("WEIGHT_DECAY = 0.05", "WEIGHT_DECAY = 0.02")], None),

    # + LR0.025 (even lower)
    ("comb: LR0.025", [("MATRIX_LR = 0.03", "MATRIX_LR = 0.025")], None),

    # + LR0.02 (way lower)
    ("comb: LR0.02", [("MATRIX_LR = 0.03", "MATRIX_LR = 0.02")], None),

    # Best combo: warmdown0.8 + WD0.03 + 10min
    ("comb: best_10min", [
        ("WARMDOWN_RATIO = 0.75", "WARMDOWN_RATIO = 0.8"),
        ("WEIGHT_DECAY = 0.05", "WEIGHT_DECAY = 0.03"),
    ], [(f"TIME_BUDGET = {time_budget}", f"TIME_BUDGET = {time_budget * 2}")]),

    # Baseline 10min for comparison
    ("comb: baseline_10min", [
...
print("\n" + "="*60)
print("SWEEP 26 COMBINATION SUMMARY")
print("="*60)
